...
const [selectedSize, setSelectedSize] = useState<string>('all');
  ...
  const { data: categoriesData } = useCategories('bhyross');
...
const colors = getAvailableColors();
  ...
  // Helper function to safely get all product images
  const getAllProductImages = (product: any) => {
    if (!product) return [];
    
    // First try product_images directly
    ...
    if (product.product_variants && Array.isArray(product.product_variants)) {
      return product.product_variants.flatMap(variant => 
        variant?.product_images && Array.isArray(variant.product_images) ? variant.product_images : []
      );
    }
...
useEffect(() => {
  ...
  return allImages[index]?.image_url || allImages[0]?.image_url || '/api/placeholder/400/400';
};
  ...
  const shouldLoadImage = useCallback((productId: string, index: number) => {
    return index < 4 || visibleProducts.has(productId);
  }, [visibleProducts]);
...
<div className="mt-6 bg-gradient-to-r from-neutral-50 to-neutral-100 p-4 rounded-lg border border-neutral-200">
  <p className="text-sm text-neutral-600 text-center">
    <span className="font-bold" style={{ color: '#6F2232' }}>{filteredProducts.length}</span> 
...
{/* Desktop Version - with image changing on hover */}
...
<div className="lg:hidden relative">
  ...
  <p className="font-bold text-base md:text-lg mb-3" style={{ color: '#6F2232' }}>
